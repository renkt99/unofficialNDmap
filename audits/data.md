...
- [ ] Run `node scripts/build-geojson.mjs` from a clean checkout and confirm
      `data/buildings.geojson` and `data/pois.geojson` come out byte-identical to
      the committed versions — any diff means the committed generated files are
      stale relative to `nd-buildings.json` / `footprints-raw.json` / `pois-raw.json`.
...
- [ ] After each `fetch-footprints.mjs` refresh, diff the new `footprints-raw.json`
      / `pois-raw.json` against the previous commit and manually review changed/
...
- [ ] Confirm the Fremantle CAT bus is still discontinued (it was as of 2023) and
      that `scripts/build-geojson.mjs` / the POI layer still correctly shows
      regular Transperth bus stops instead of a defunct CAT layer — re-check if
      the service is ever reinstated.
